...
import { addToCart, createCardItem } from "./cart";
...
export const createProductCard = ({
  ...
  const isExitedInCart = cartBody.querySelector(`[cart-product-id='${id}']`)
...
export const productListHandler = (event) => {
  if (event.target.classList.contains("add-to-cart")) {
    ...
    const currentAddToCartBtn = currentProductCard.querySelector(".add-to-cart");    // console.log(currentProductCard);
    ...
    if(cartBox.classList.contains("translate-x-full")){
      ...
      const lastCardPosition = document.querySelector(".cart-item:last-child")
      ?.getBoundingClientRect();

      const aniTop = lastCardPosition ? lastCardPosition.top + 10 : cartBody.getBoundingClientRect.top;
      const aniLeft = lastCardPosition ? lastCardPosition.left + 10 : cartBody.getBoundingClientRect.left;
       keyframe = [
        {
          top: currentImgPosition.top + "px",
          left: currentImgPosition.left + "px",
